...
const storageGet = (key, def) =>
  localStorage.getItem("data-viewer:" + key) || def;
const storageSet = (key, val) =>
  localStorage.setItem("data-viewer:" + key, val);
...
const vue = Vue.createApp({
  data() {
    return {
      ...
      entity: storageGet("entity")
        ? JSON.parse(storageGet("entity"))
        : undefined,
      entities: [],
      columns: [],
      data: [],
  ...
  watch: {
    dataSource: (v) => {
      storageSet("data-source", v);
      vue.fetchEntities();
    },
    skip: (v) => {
      storageSet("skip", v);
      if (vue.entity) vue.fetchData();
    },
    top: (v) => {
      storageSet("top", v);
      if (vue.entity) vue.fetchData();
  ...
  methods: {
    async fetchEntities() {
      ...
      const entity =
        vue.entity && vue.entities.find((e) => e.name === vue.entity.name);
      if (entity) {
        // restore selection from previous fetch
    ...
    async inspectEntity(eve) {
      const entity = (vue.entity =
        vue.entities[eve.currentTarget.rowIndex - 1]);
    ...
    async fetchData() {
      ...
      try {
        ...
        vue.data = data.value.map((d) =>
          d.record
            .sort(
              (r1, r2) => columnIndexes[r1.column] - columnIndexes[r2.column]
            )
            .map((r) => r.data)
        );
        const row = vue.data.find(
          (data) => vue._makeRowKey(data) === vue.rowKey
        );
    ...
    _makeRowKey(row) {
      ...
      return row
        .filter(
          (_, colIndex) => vue.columns[colIndex] && vue.columns[colIndex].isKey
        )
